...
export function createApp() {
    ...
    app.post('/proactive/register', async (c) => {
        ...
        //   正常几 KB~几十 KB；给 1MB 上限足够，超了拒绝（多半是异常/恶意输入）。
        if (typeof promptTemplate === 'string' && promptTemplate.length > 1024 * 1024) {
            return c.json({ error: 'promptTemplate too large (>1MB)' }, 413);
        }
